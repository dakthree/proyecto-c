...
// Estado global en el isolate (igual que en server.js)
// Persiste entre requests mientras el Worker esté caliente
...
export { candidates, youtubeHandleCache, twitchLoginCache };

export function getClipStartDate(env) {
  return env.CLIP_START_DATE || '2026-08-11T10:00:00Z';
}

export function getTwitchPzGameIdFromEnv(env) {
  return env.TWITCH_PZ_GAME_ID || null;
}

export function getClipCache() { return clipCache; }
export function setClipCache(v) { clipCache = v; }

export async function twitchAppToken(env) {
...
export async function twitchLive(env) {
...
export async function youtubeChannelId(ref, env) {
  if (!ref) return null;
...
export async function youtubeLiveForChannel(channelId, candidate, env) {
...
export async function youtubeLive(env) {
  ...
  const results = await Promise.all(ytCandidates.map(async candidate => {
    try {
      ...
      const channelId = await youtubeChannelId(ref, env);
      if (!channelId) return null;
      return youtubeLiveForChannel(channelId, candidate, env);
    } catch (error) {
...
export async function twitchProjectZomboidGameId(token, env){
  if(twitchPzGameIdCache) return twitchPzGameIdCache;
  const params=new URLSearchParams({query:'Project Zomboid'});
  const res=await fetch(`https://api.twitch.tv/helix/search/categories?${params}`,{headers:{'Client-Id':env.TWITCH_CLIENT_ID,'Authorization':`Bearer ${token}`}});
  ...
  twitchPzGameIdCache=exact?.id || getTwitchPzGameIdFromEnv(env) || '31339';
  return twitchPzGameIdCache;
}

export async function twitchUsersByLogin(logins, env){
...
export function weekWindows(startIso,endDate){
...
export async function twitchClips(env){
  if(!env.TWITCH_CLIENT_ID||!env.TWITCH_CLIENT_SECRET) return[];
  const tp=candidates.filter(x=>x.twitch);
  if(!tp.length) return[];

  const users=await twitchUsersByLogin(tp.map(x=>String(x.twitch).split('/').filter(Boolean).pop()), env);
...
export async function youtubeClips(env){
  if(!env.YOUTUBE_API_KEY) return[]; const out=[];
  const CLIP_START_DATE = getClipStartDate(env);
  for(const c of candidates.filter(x=>x.youtube)){
    try{
      const id=await youtubeChannelId(parseYoutube(c.youtube), env); if(!id)continue;
      const url=new URL('https://www.googleapis.com/youtube/v3/search');
      url.searchParams.set('part','snippet'); url.searchParams.set('channelId',id); url.searchParams.set('type','video');
      url.searchParams.set('publishedAfter',CLIP_START_DATE); url.searchParams.set('order','date'); url.searchParams.set('maxResults','25'); url.searchParams.set('key',env.YOUTUBE_API_KEY);
